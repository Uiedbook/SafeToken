...
export class SafeToken {
  // ? full token
  private FA_token: string;
  private FR_token: string;
  ...
  // ? FR key
  private rtStoreKey: string = "_safetoken";
  private key: string;
  constructor(init?: {
    timeWindow?: number;
    rtDays?: number;
    ...
    // ? refresh file name
    if (init?.rtStoreKey) this.rtStoreKey = init.rtStoreKey;
    //? setup encryption keys
    this.key = init?.encryptionKey || "";
    // ? retrieve last refresh tokens
    [this.lastrefreshTime, this.FR_token] = SafeToken.retrToken(
      this.rtStoreKey
    );
  }
  ...
  verifyAccessToken(hashString: string, _r = false): string | boolean {
    ...
    const key = (_r ? this.FR_token : this.FA_token).slice(
      Number(si) - 10,
      Number(si)
    );
  ...
  resetRefreshToken() {
    ...
    writeFileSync(
      this.rtStoreKey,
      this.lastrefreshTime + ":" + ":" + this.FR_token
    );
  }
